...
void
open_sharedmem(int _id) {
  acquire(&sh_table.lock);
  struct proc* proc = myproc();
  pde_t *pgdir = proc->pgdir;
  uint size = proc->sz;
  for(int i = 0; i < MAX_PAGES; i++) {
    if (sh_table.pages[i]->id == _id) {
      mappages(pgdir, (char*)PGROUNDUP(size), PGSIZE, (uint)V2P(sh_table.pages[i]->frame), PTE_W|PTE_U);
      sh_table.pages[i]->ref_cnt++;
  ...
  mem = kalloc();
  memset(mem, 0, PGSIZE);
  mappages(pgdir, (char*)PGROUNDUP(size), PGSIZE, (uint)V2P(mem), PTE_W|PTE_U);
  sh_table.pages[sh_table.pages_cnt]->frame = mem;
  sh_table.pages[sh_table.pages_cnt]->ref_cnt++;
  sh_table.pages[sh_table.pages_cnt]->id = _id;
  sh_table.pages_cnt++;
  release(&sh_table.lock);
}

void
close_sharedmem(int _id) {
  struct proc* proc = myproc();
  acquire(&sh_table.lock);
  for(int i = 0; i < MAX_PAGES; i++) {
    if(sh_table.pages[i]->id == _id) {
      sh_table.pages[i]->ref_cnt--;
      if (sh_table.pages[i]->ref_cnt == 0) {
        uint* pte = walkpgdir(proc->pgdir, sh_table.pages[i]->frame, 0);
        uint addr = PTE_ADDR(*pte);
        kfree((char*)addr);
      }
      release(&sh_table.lock);
      return;
